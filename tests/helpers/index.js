...
async function get(router, url, prodUrl) {
  ...
  if (process.env.TEST_MODE === 'COMPONENT') {
    ...
    console.log('==>', `calling the router directly at ${url}`);
    ...
    console.log('==>', `calling the router via API connect URL: ${config.APIConnect.url}${apiUrl}`);
    response = await rp({
    ...
    console.log('response from router ===>', response.statusCode, response.body);
  }
  return response;
}
